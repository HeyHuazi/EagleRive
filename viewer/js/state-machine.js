...
const StateMachine = (function () {
  ...
  function populateStateMachines(riveInstance) {
    ...
    sel.innerHTML = sms.length
      ...
      : '<option value="">' +
        (window.i18n ? window.i18n.t('messages.noStateMachines') : '暂无状态机') +
        '</option>';
    ...
    sel.onchange = () => {
      if (sel.value) {
        ...
        requestAnimationFrame(() =>
          requestAnimationFrame(() => loadSMInputs(riveInstance, sel.value))
        );
      }
    };
  ...
  function loadSMInputs(riveInstance, name) {
    ...
    try {
      ...
      for (let i = 0; i < inputs.length; i++) {
        ...
        if (type === rive.StateMachineInputType.Trigger) triggers.push(inp);
        else if (type === rive.StateMachineInputType.Boolean) bools.push(inp);
        else if (type === rive.StateMachineInputType.Number) nums.push(inp);
      }

      let html = '';

      if (triggers.length) {
        const triggerLabel = window.i18n ? window.i18n.t('stateMachine.triggers') : '触发器';
        html +=
          '<div class="sm-type-group"><div class="sm-type-hd">' +
          triggerLabel +
          ' <span class="sm-type-badge">' +
          triggers.length +
          '</span></div><div class="trigger-grid">';
        triggers.forEach(function (t) {
          html +=
            '<button class="trigger-btn" data-trigger="' +
            RiveUtils.escapeAttr(t.name) +
            '">' +
            RiveUtils.escapeAttr(t.name) +
            '</button>';
        });
      ...
      if (bools.length) {
        const booleanLabel = window.i18n ? window.i18n.t('stateMachine.booleans') : '布尔值';
        html +=
          '<div class="sm-type-group"><div class="sm-type-hd">' +
          booleanLabel +
          ' <span class="sm-type-badge">' +
        ...
        bools.forEach(function (b) {
          html +=
            '<div class="icard"><div class="sw-row"><span class="icard-label" style="margin:0">' +
      ...
      if (nums.length) {
        const numberLabel = window.i18n ? window.i18n.t('stateMachine.numbers') : '数值';
        html +=
          '<div class="sm-type-group"><div class="sm-type-hd">' +
          numberLabel +
          ' <span class="sm-type-badge">' +
        ...
        nums.forEach(function (n) {
          html +=
            '<div class="icard"><label class="icard-label">' +
            RiveUtils.escapeAttr(n.name) +
  ...
  function bindSMEvents(riveInstance) {
    ...
    box.querySelectorAll('[data-trigger]').forEach(function (el) {
      el.addEventListener('click', function () {
        ...
        } else console.warn('[Rive] SM trigger not found:', this.dataset.trigger);
      });
    });
